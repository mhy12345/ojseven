<?php
   ...
   $result0=mysql_query("SELECT * FROM schedule WHERE activate_time<=now() AND done=FALSE AND action='contest-make'");
   while ($row0=mysql_fetch_array($result0))
   {
	  ...
	  if ($cnt==5)
	  mysql_query("CREATE TABLE ".$tname."(uid int primary key,rank int,score int,s1 int,s2 int,s3 int,s4 int,s5 int)");
	  $result=mysql_query("SELECT * FROM statuses WHERE contest=".$cid." and user!=13");
