...
void check_status(MYSQL &mysql)
{
		...
		while ((row=mysql_fetch_row(result))!=NULL)
		{
				...
				if (issubmit)
				{
						sprintf(cc,"unzip %s/status/%d/answer.zip -d %s",home,sid,rundir);system(cc);
						printf("%s\n",cc);
				...
				{
						sprintf(cc,"cp ./status/%d/%s %s/%s\n",sid,pfilename,juddir,pfilename);system(cc);
						sprintf(cc,"cp ./problems/%d/data.cfg %s\n",pid,juddir);system(cc);
						sprintf(cc,"%s/constraint.txt",juddir);
						FILE *cfile=fopen(cc,"w");
				...
				sprintf(cc,"%s/status/%d/result.txt",home,sid);
